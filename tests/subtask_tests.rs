...
fn test_remove_parent_removes_subtasks() {
    ...
    let _subtask1_id = todo_list
        .add_subtask(parent_id, "Subtask 1".to_string())
        .unwrap();
    let _subtask2_id = todo_list
...
fn test_complete_parent_independent_of_subtasks() {
    ...
    let parent_id = todo_list.add_task(TaskWithoutId::new("Parent task".to_string()));
    let subtask1_id = todo_list
        .add_subtask(parent_id, "Subtask 1".to_string())
        .unwrap();
    let subtask2_id = todo_list
        .add_subtask(parent_id, "Subtask 2".to_string())
        .unwrap();
    ...
    assert!(!subtasks
        .iter()
        .any(|t| t.id == subtask1_id && t.is_completed()));
    assert!(!subtasks
        .iter()
        .any(|t| t.id == subtask2_id && t.is_completed()));
...
fn test_many_subtasks_single_parent() {
    ...
    let subtask_ids: Vec<usize> = todo_list
...
fn test_subtask_removal_edge_cases() {
    ...
    let sub1_1 = todo_list
        .add_subtask(parent1, "Sub 1.1".to_string())
        .unwrap();
    let sub1_2 = todo_list
        .add_subtask(parent1, "Sub 1.2".to_string())
        .unwrap();

    // Create parent2 with subtasks
    let parent2 = todo_list.add_task(TaskWithoutId::new("Parent 2".to_string()));
    let _sub2_1 = todo_list
        .add_subtask(parent2, "Sub 2.1".to_string())
        .unwrap();
