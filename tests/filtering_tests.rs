...
fn test_empty_filter_results() {
    ...
    assert_eq!(
        tasks.len(),
        0,
        "Should have no completed low priority tasks"
    );
}
...
fn test_priority_filter_mixed_states() {
    ...
    assert_eq!(
        medium_tasks.len(),
        4,
        "Should get all medium priority tasks"
    );

    // Filter by priority + completed
    let completed_medium = TaskFilter::all()
        .with_status(TaskStatus::Completed)
        .with_priority(Priority::Medium);
    ...
    assert_eq!(
        completed_tasks.len(),
        2,
        "Should get 2 completed medium tasks"
    );
...
fn test_filter_dynamic_updates() {
    ...
    let completed_filter = TaskFilter::all()
        .with_status(TaskStatus::Completed)
        .with_priority(Priority::High);
    let completed_tasks = todo_list.get_filtered_tasks(&completed_filter);
