...
impl<O: OutputWriter> ProjectCommandOutputManager<O> {
    ...
    pub fn show_project_already_exists(&mut self, name: &str) {
        self.output_writer.borrow_mut().show_error(&format!("Project '{}' already exists", name));
    }

    /// Displays an error when a project is not found.
    pub fn show_project_not_found(&mut self, name: &str) {
        self.output_writer.borrow_mut().show_error(&format!("Project '{}' not found", name));
    }

    /// Displays an error when trying to delete the current project.
    pub fn show_cannot_delete_current_project(&mut self, name: &str) {
        self.output_writer.borrow_mut().show_error(&format!("Cannot delete current project '{}'. Switch to another project first.", name));
    }
...
mod tests {
    ...
    fn test_show_projects() {
        ...
        let projects = vec!["Work".to_string(), "Personal".to_string(), "default".to_string()];
