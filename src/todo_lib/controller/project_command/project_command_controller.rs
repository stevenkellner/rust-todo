...
impl<O: OutputWriter> ProjectCommandController<O> {
    ...
    fn handle_command(&mut self, command: &ProjectCommand) -> CommandControllerResult {
        match command {
            ...
            ProjectCommand::RenameProject(old_name, new_name) => self.rename_project(old_name, new_name),
        }
    }

    fn new_project(&mut self, name: &str) -> CommandControllerResult {
        match self.project_manager.borrow_mut().create_project(name.to_string()) {
            Some(()) => {
    ...
    fn switch_project(&mut self, name: &str) -> CommandControllerResult {
        match self.project_manager.borrow_mut().switch_project(name.to_string()) {
            Some(()) => {
    ...
    fn list_projects(&mut self) -> CommandControllerResult {
        ...
        self.output_manager.show_projects(&projects, current_project);
        CommandControllerResult::default()
    }

    fn delete_project(&mut self, name: &str) -> CommandControllerResult {
        let current_project = self.project_manager.borrow().get_current_project_name().to_string();
        ...
        match self.project_manager.borrow_mut().delete_project(name.to_string()) {
            Some(()) => {
    ...
    fn rename_project(&mut self, old_name: &str, new_name: &str) -> CommandControllerResult {
        match self.project_manager.borrow_mut().rename_project(old_name.to_string(), new_name.to_string()) {
            Some(()) => {
                self.output_manager.show_project_renamed(old_name, new_name);
                CommandControllerResult::with_action(CommandControllerResultAction::SaveTodoList)
            }
            None => {
                if self.project_manager.borrow().list_projects().contains(&new_name.to_string()) {
...
mod tests {
    ...
    fn test_new_project() {
        ...
        let mut controller = ProjectCommandController::new(Rc::clone(&project_manager), output_writer);
    ...
    fn test_rename_project() {
        ...
        assert!(result.has_action(&CommandControllerResultAction::SaveTodoList));
        assert_eq!(project_manager.borrow().get_current_project_name(), "personal");
    }
}
