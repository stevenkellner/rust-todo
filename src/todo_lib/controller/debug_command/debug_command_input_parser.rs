...
impl DebugCommandInputParser {
    ...
    pub fn try_parse(
        ...
        command: &str,
        args: &[&str],
    ) -> Option<Result<DebugCommand, ParseError>> {
        match command {
    ...
    fn parse_debug_generate_command(&self, args: &[&str]) -> Result<DebugCommand, ParseError> {
        if args.is_empty() {
            ...
            Err(ParseError::InvalidId(
                "Invalid count. Please provide a number.".to_string(),
            ))
        }
    }

    /// Parses the 'debug:gen-projects' command to generate random projects with tasks.
    fn parse_debug_generate_projects_command(
        &self,
        args: &[&str],
            ...
            (args[0].parse::<usize>(), args[1].parse::<usize>())
        {
            if project_count == 0 {
                ...
                Ok(DebugCommand::GenerateProjects {
                    project_count,
                    tasks_per_project,
                })
            }
            ...
            Err(ParseError::InvalidId(
                "Invalid count. Please provide numbers.".to_string(),
            ))
        }
    }
}
